...
import {
  fetchAdminCommunityConfig,
  updateAdminCommunityConfig,
...
export default function CommunityConfig() {
  ...
  const configKey = queryKeys.admin.communityConfig(user?.communityId ?? "");

  const { data: configData, isLoading: initialLoad } = useQuery({
    queryKey: configKey,
    queryFn: () =>
      fetchAdminCommunityConfig(token, user!.communityId as string),
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: object) => updateAdminCommunityConfig(token, payload),
    onSuccess: (res: any) => {
  ...
  const selColor = selFac
    ? FACILITY_COLORS[
        FACILITY_TYPES.findIndex((f) => f.id === selFac.id) %
          FACILITY_COLORS.length
      ]
    : tint;
  const [startTime, endTime] = (
    selConfig.operatingHours || "09:00-21:00"
  ).split("-");
        ...
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            <Text style={[styles.screenSub, { color: muted }]}>
              {enabledCount} facilities enabled
            </Text>
          </View>
        </View>
        <TouchableOpacity
          onPress={handleSave}
          disabled={saveMutation.isPending || !communityData.name.trim()}
          style={[
            styles.saveBtn,
            {
              backgroundColor: tint,
              opacity:
                saveMutation.isPending || !communityData.name.trim() ? 0.5 : 1,
            },
          ]}
        >
      ...
      >
        ...
        >
          <View style={styles.sectionHead}>
            <View
              style={[styles.sectionIcon, { backgroundColor: tint + "1A" }]}
            >
        ...
        >
          <View style={styles.sectionHead}>
            <View
              style={[styles.sectionIcon, { backgroundColor: "#10B9811A" }]}
            >
            ...
            <View>
              ...
              <Text style={[styles.sectionSub, { color: muted }]}>
                Tap to configure · {enabledCount} enabled
              </Text>
            </View>
          </View>
          ...
          <View style={styles.facilityGrid}>
            {FACILITY_TYPES.map((ft, i) => {
              ...
              return (
                <TouchableOpacity
                  key={ft.id}
                  onPress={() => setSelectedFacility(ft)}
                  style={[
                    styles.facilityCard,
                    {
                ...
                >
                  {isOn && (
                    <View
                      style={[styles.facilityCheck, { backgroundColor: color }]}
                    >
                  ...
                  >
                    <Feather
                      name={ft.icon}
                      size={18}
                      color={isOn ? color : muted}
                    />
                  </View>
                  <Text
                    style={[
                      styles.facilityName,
                      { color: isOn ? color : textColor },
                    ]}
                    numberOfLines={2}
                  >
                    {ft.name}
              ...
              <>
                    ...
                    <View>
                      ...
                      <TouchableOpacity
                        ...
                        style={[
                          styles.togglePill,
                          {
                            backgroundColor: selConfig.enabled
                              ? "#10B98120"
                              : "#EF444420",
                          },
                        ]}
                      >
                        <Feather
                          name={
                            selConfig.enabled ? "toggle-right" : "toggle-left"
                          }
                          size={14}
                          color={selConfig.enabled ? "#10B981" : "#EF4444"}
                        />
                        <Text
                          style={[
                            styles.toggleText,
                        ...
                        >
                          {selConfig.enabled ? "Enabled" : "Disabled"}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                ...
                >
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          updateFacility(
                            selFac.id,
                            "quantity",
                            Math.max(1, (selConfig.quantity || 1) - 1),
                          )
                        }
                        style={[styles.counterBtn, { borderColor: borderCol }]}
                      >
                        <Feather name="minus" size={14} color={textColor} />
                      </TouchableOpacity>
                      <Text style={[styles.counterVal, { color: textColor }]}>
                        {selConfig.quantity || 1}
                      </Text>
                      <TouchableOpacity
                        onPress={() =>
                          updateFacility(
                            selFac.id,
                            "quantity",
                            Math.min(50, (selConfig.quantity || 1) + 1),
                          )
                        }
                        style={[styles.counterBtn, { borderColor: borderCol }]}
                      >
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          updateFacility(
                            selFac.id,
                            "maxCapacity",
                            Math.max(1, (selConfig.maxCapacity || 10) - 1),
                          )
                        }
                        style={[styles.counterBtn, { borderColor: borderCol }]}
                      >
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          updateFacility(
                            selFac.id,
                            "maxCapacity",
                            Math.min(1000, (selConfig.maxCapacity || 10) + 1),
                          )
                        }
                        style={[styles.counterBtn, { borderColor: borderCol }]}
                      >
                  ...
                  <View style={styles.modalSection}>
                    <Text style={[styles.modalSectionTitle, { color: muted }]}>
                      Operating Hours
                    </Text>
                    <View style={styles.hoursRow}>
                      ...
                      <Text style={[styles.hoursSep, { color: muted }]}>
                        to
                      </Text>
                      <View
                        style={[
                  ...
                  <View style={styles.modalRow}>
                    <View>
                      <Text
                        style={[styles.modalRowLabel, { color: textColor }]}
                      >
                        Paid Facility
                      </Text>
                      <Text style={[styles.modalRowSub, { color: muted }]}>
                        Charge residents for booking
                      </Text>
                    </View>
                    <Switch
                      value={selConfig.isPaid || false}
                      onValueChange={(v) =>
                        updateFacility(selFac.id, "isPaid", v)
                      }
                      trackColor={{ false: borderCol, true: selColor + "80" }}
                      thumbColor={selConfig.isPaid ? selColor : muted}
                    />
                  </View>
                  ...
                  {selConfig.isPaid && (
                    <View style={styles.modalSection}>
                      <Text
                        style={[styles.modalSectionTitle, { color: muted }]}
                      >
                        Pricing
                      </Text>
                      <View
                      ...
                      >
                        <Text style={[styles.currencyLabel, { color: muted }]}>
                          ₹
                        </Text>
                        <TextInput
                      ...
                      >
                        <Picker
                          selectedValue={selConfig.priceType || "per_hour"}
                          onValueChange={(v) =>
                            updateFacility(selFac.id, "priceType", v)
                          }
                          style={{ color: textColor }}
                          dropdownIconColor={muted}
                        >
                  ...
                  <View style={styles.fieldWrap}>
                    ...
                    >
                      ...
                      <TextInput
                        ...
                        placeholder="Any rules for this facility..."
                        placeholderTextColor={muted}
                        value={selConfig.rules || ""}
                        onChangeText={(v) =>
                          updateFacility(selFac.id, "rules", v)
                        }
                        multiline
                      />
                    </View>
                ...
                <View
                  style={[styles.modalFooter, { borderTopColor: borderCol }]}
                >
                  <TouchableOpacity
                    onPress={() => setSelectedFacility(null)}
