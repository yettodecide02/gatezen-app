...
// --- Allowed status transitions (strictly forward) ---
...
function MaintenanceCard({
  ...
  return (
    ...
    >
      ...
      <View style={styles.mCardTop}>
        ...
        <View style={styles.mCardInfo}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            <Text
              style={[styles.mCardTitle, { color: textColor }]}
              numberOfLines={1}
            >
          ...
          >
            ...
            <Text style={[styles.mMeta, { color: muted }]}>
      ...
      {/* Description */}
      {item.description && (
        <Text style={[styles.mDesc, { color: muted }]} numberOfLines={2}>
          {item.description}
        </Text>
      )}
        ...
        {Object.entries(STATUS_CONFIG).map(([status, cfg]) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                styles.mActionBtn,
                { backgroundColor: cfg.bg, opacity: updateLoading ? 0.5 : 1 },
              ]}
              onPress={() => onStatusUpdate(item.id, status)}
              disabled={!!updateLoading}
            >
              <Feather name={cfg.icon} size={12} color={cfg.color} />
              <Text style={[styles.mActionBtnText, { color: cfg.color }]}>
                {updateLoading ? "..." : cfg.label}
              </Text>
            </TouchableOpacity>
          );
        })}
      ...
      {/* Comments Toggle */}
      <TouchableOpacity
        style={[styles.commentToggle, { borderTopColor: borderCol }]}
        onPress={() => setShowComments((v) => !v)}
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          <Feather name="message-circle" size={13} color={tint} />
          <Text style={[styles.commentToggleText, { color: tint }]}>
