...
export default function OverstaySettings() {
  ...
  const { data: fetchedLimits, isLoading: loading } = useQuery({
    queryKey: ["admin", "overstayLimits"],
    queryFn: fetchOverstayLimits,
    staleTime: 30 * 60 * 1000,
  });

  useEffect(() => {
    if (!fetchedLimits) return;
    setLimits(fetchedLimits);
    setOriginal(fetchedLimits);
    const iv: Record<string, string> = {};
    Object.entries(fetchedLimits).forEach(([k, v]) => {
      iv[k] = String(v);
    });
    setInputVals(iv);
  }, [fetchedLimits]);
  ...
  const saveMutation = useMutation({
    mutationFn: () => saveOverstayLimits(limits),
    onSuccess: (result) => {
      if (result.ok) {
        setOriginal({ ...limits });
    ...
    onError: () => showError("Failed to save. Please try again."),
  });
  ...
  const handleReset = () => {
    ...
    const iv: Record<string, string> = {};
    Object.entries(DEFAULT_OVERSTAY_LIMITS).forEach(([k, v]) => {
      iv[k] = String(v);
    });
    setInputVals(iv);
  };
    ...
    <View style={{ flex: 1, backgroundColor: pageBg }}>
        ...
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            <Text style={[styles.headerSub, { color: muted }]}>
              Set allowed time per visitor type
            </Text>
          </View>
        </View>
      ...
      >
        ...
        >
          ...
          >
            Changes apply immediately to all new overstay checks. Existing
            ...
            >
              ...
              <View style={styles.cardHeader}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    <Text style={[styles.typeLabel, { color: textColor }]}>
                      {t.label}
                    </Text>
                    {changed && (
                      <View
                      ...
                      >
                        <Text
                          style={[styles.changedBadgeText, { color: t.color }]}
                        >
                          Edited
                        </Text>
                      </View>
                  ...
                  <Text style={[styles.typeSub, { color: muted }]}>
                    {t.sub}
                  </Text>
                </View>
                ...
                >
                  <Text style={[styles.currentValText, { color: t.color }]}>
                    {formatDuration(val)}
                  </Text>
                </View>
              </View>
              ...
              >
                      ...
                      backgroundColor:
                        val <= MIN_LIMIT
                          ...
                          : isDark
                            ...
                            : t.bg,
                      borderColor:
                        val <= MIN_LIMIT ? borderCol : t.color + "40",
                    },
                  ]}
                >
                  <Feather
                    name="minus"
                    size={18}
                    color={val <= MIN_LIMIT ? muted : t.color}
                  />
                </TouchableOpacity>
                ...
                <View style={styles.stepInputWrap}>
                  <TextInput
                    ...
                    selectTextOnFocus
                  />
                  <Text style={[styles.stepUnit, { color: muted }]}>
                    minutes
                  </Text>
                </View>
                      ...
                      backgroundColor:
                        val >= MAX_LIMIT
                          ...
                          : isDark
                            ...
                            : t.bg,
                      borderColor:
                        val >= MAX_LIMIT ? borderCol : t.color + "40",
                    },
                  ]}
                >
                  <Feather
                    name="plus"
                    size={18}
                    color={val >= MAX_LIMIT ? muted : t.color}
                  />
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.presetsRow}>
                <Text style={[styles.presetsLabel, { color: muted }]}>
                  Quick:
                </Text>
                {presets.map((p) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      {
                        backgroundColor:
                          val === p ? t.color : isDark ? t.darkBg : t.bg,
                        borderColor:
                          val === p ? t.color : isDark ? t.darkBg : t.bg,
                      },
                    ]}
              ...
              {changed && (
                <View
                  style={[styles.originalHint, { borderTopColor: borderCol }]}
                >
                  ...
                  <TouchableOpacity
                    onPress={() => setLimit(t.key, original[t.key])}
                  >
                    <Text
                      style={{
                        fontSize: 11,
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.resetLabel, { color: textColor }]}>
              Reset to Defaults
            </Text>
