...
function getLimit(limits, type) {
  return (
    limits[type?.toUpperCase()] ?? limits.OTHER ?? DEFAULT_OVERSTAY_LIMITS.OTHER
  );
}
...
function isOverstay(visitor, limits) {
  ...
  return (
    getOverstayMins(visitor.checkInAt) > getLimit(limits, visitor.visitorType)
  );
}
...
function OverstayCard({
  ...
  const type =
    TYPE_CONFIG[visitor.visitorType?.toUpperCase()] || TYPE_CONFIG.OTHER;
    ...
    >
      ...
      <View style={styles.cardTop}>
        <View
          style={[
            styles.typeIcon,
            { backgroundColor: isDark ? type.darkBg : type.bg },
          ]}
        >
      ...
      >
        ...
        <Text style={[styles.durationText, { color: urgency.color }]}>
          Inside for{" "}
          <Text style={{ fontWeight: "800" }}>{formatDuration(totalMins)}</Text>
          {"  ·  "}Overstay:{" "}
          <Text style={{ fontWeight: "800" }}>{formatDuration(ovMins)}</Text>
      ...
      <View style={styles.metaRow}>
        <View style={styles.metaItem}>
          ...
          <Text style={[styles.metaText, { color: muted }]}>
            Checked in {formatTime(visitor.checkInAt)}
          </Text>
        </View>
        {visitor.contact && (
          <View style={styles.metaItem}>
...
function TabBtn({ label, count, active, onPress, isDark, muted }) {
  return (
    ...
    >
      <Text style={[styles.tabBtnText, { color: active ? "#FFF" : muted }]}>
        {label}
      </Text>
      {count > 0 && (
        <View
...
export default function AdminOverstayAlert() {
  ...
  // Load limits once on mount
  useEffect(() => {
    fetchOverstayLimits().then((data) => setLimits(data));
  }, []);
        ...
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            <Text style={[styles.headerSub, { color: muted }]}>
              {loading
                ? "Loading..."
                : `${allOverstays.length} active alert${allOverstays.length !== 1 ? "s" : ""}`}
            </Text>
          </View>
        </View>
        ...
        >
          ...
          {allOverstays.length > 0 && (
            ...
            >
              ...
              >
                {[
                  byType.DELIVERY.length > 0
                    ? `${byType.DELIVERY.length} delivery`
                    : "",
                  byType.GUEST.length > 0 ? `${byType.GUEST.length} guest` : "",
                  byType.STAFF.length > 0 ? `${byType.STAFF.length} staff` : "",
                  byType.CAB_AUTO.length > 0
                    ? `${byType.CAB_AUTO.length} cab`
                    : "",
                ]
                  .filter(Boolean)
                  .join(" · ")}{" "}
                overstaying right now
              </Text>
            </View>
          )}
          ...
          >
            ...
            >
              ...
              <Text style={[styles.limitsTitle, { color: textColor }]}>
                Overstay Limits
              </Text>
              <View style={{ flex: 1 }} />
              <View
              ...
              >
                ...
                <Text style={[styles.editChipText, { color: muted }]}>
                  Edit Limits
                </Text>
              </View>
            </View>
