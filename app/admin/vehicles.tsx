...
import { fetchAdminVehicles, updateAdminVehicle } from "@/lib/queries/admin";
...
export default function AdminVehicles() {
  ...
  const {
    ...
    queryKey: [...queryKeys.admin.vehicles(communityId ?? ""), activeTab],
    queryFn: () => fetchAdminVehicles(token, communityId, activeTab),
    enabled: !!communityId,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateAdminVehicle(token, id, payload),
    onSuccess: (_, { payload }) => {
      const status = payload.status;
      showSuccess(
        status === "APPROVED" ? "Vehicle approved." : "Vehicle rejected.",
      );
      queryClient.invalidateQueries({
        queryKey: queryKeys.admin.vehicles(communityId ?? ""),
      });
    },
    onError: (_, { payload }) =>
      showError(
    ...
    updateMutation.mutate(
      {
        ...
        payload: {
          status: "REJECTED",
          rejectionReason: rejectReason.trim() || "Not approved",
        },
      },
      {
        onSettled: () => {
        ...
        {TABS.map((t) => (
          <Pressable
            ...
            style={{
              ...
              paddingVertical: 7,
              borderRadius: 20,
              backgroundColor: activeTab === t ? tint : "transparent",
              borderWidth: 1,
              borderColor: activeTab === t ? tint : borderCol,
            }}
          >
            <Text
              style={{
          ...
          {vehicles.length === 0 ? (
            ...
            >
              ...
              <Text style={{ fontSize: 15, color: muted }}>
                No {STATUS_CONFIG[activeTab].label.toLowerCase()} vehicles
              </Text>
            </View>
          ) : (
            vehicles.map((v) => (
