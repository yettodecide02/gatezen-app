...
export default function ResidentForm() {
  ...
  useEffect(() => {
    if (step === 2 && communities.length === 0) fetchCommunities();
  }, [step]);
  ...
  const fetchBlocks = async (cId) => {
    ...
    try {
      const res = await axios.get(
        `${config.backendUrl}/auth/communities/${cId}/blocks`,
      );
  ...
  const fetchUnits = async (bId) => {
    ...
    try {
      const res = await axios.get(
        `${config.backendUrl}/auth/blocks/${bId}/units`,
      );
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      if (res.data.success) showSuccess("OTP resent");
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text style={{ fontSize: 14, color: text }}>
                    {item[labelKey] || item.number || item.name}
                  </Text>
                </Pressable>
              )}
            />
          ...
          {[1, 2].map((s) => (
            <View
              ...
              style={{
                ...
                flex: s === step ? 2 : 1,
                borderRadius: 2,
                backgroundColor: s <= step ? tint : borderCol,
              }}
            />
          ))}
        </View>
        ...
        {step === 1 ? (
          <>
            ...
            <Pressable
              ...
              disabled={loading}
              style={({ pressed }) => ({
                backgroundColor: pressed || loading ? tint + "CC" : tint,
                borderRadius: 12,
                padding: 14,
                alignItems: "center",
              })}
            ...
            >
              <Text style={{ fontSize: 13, color: tint, fontWeight: "600" }}>
                Resend Code
              </Text>
            </Pressable>
          </>
          ...
          <>
            <View style={{ gap: 4 }}>
              ...
              <Text style={{ fontSize: 13, color: muted }}>
                Choose your community, block, and unit
              </Text>
            </View>
              ...
              >
                ...
                >
                  ...
                  <Text
                    style={{
                      fontSize: 14,
                      color: communityName ? text : muted,
                    }}
                  >
            ...
            {selectedCommunity !== "" && (
              <View>
                ...
                <Pressable
                  onPress={() => blocks.length > 0 && setShowBlockModal(true)}
                  style={{
                    flexDirection: "row",
                    alignItems: "center",
                    justifyContent: "space-between",
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      style={{ fontSize: 14, color: blockName ? text : muted }}
                    >
                      {blockName || "Select block (optional)"}
                    </Text>
                  </View>
                  {loadingBlocks ? (
                    <ActivityIndicator size="small" color={tint} />
            ...
            {selectedBlock !== "" && (
              <View>
                ...
                <Pressable
                  onPress={() => units.length > 0 && setShowUnitModal(true)}
                  style={{
                    flexDirection: "row",
                    alignItems: "center",
                    justifyContent: "space-between",
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      style={{ fontSize: 14, color: unitName ? text : muted }}
                    >
                      {unitName || "Select unit (optional)"}
                    </Text>
                  </View>
                  {loadingUnits ? (
                    <ActivityIndicator size="small" color={tint} />
            ...
            <Pressable
              ...
              style={({ pressed }) => ({
                backgroundColor:
                  pressed || loading || !selectedCommunity ? tint + "80" : tint,
                borderRadius: 12,
                padding: 14,
                alignItems: "center",
                marginTop: 4,
