...
import {
  fetchResidentElectionPolls,
  createResidentElectionPoll,
  voteOnResidentElectionPoll,
  deleteResidentElectionPoll,
      ...
      <View style={styles.pollBody}>
        ...
        <View style={[styles.pollStats, { borderColor: borderCol }]}>
          ...
          <View style={styles.pollStatItem}>
            <Text
              style={[
                styles.pollStatValue,
                { color: hasVoted ? "#10B981" : muted },
              ]}
            >
              {hasVoted ? "✓" : "—"}
            </Text>
            <Text style={[styles.pollStatLabel, { color: muted }]}>
              {hasVoted ? "Voted" : "Your Vote"}
            </Text>
          </View>
        </View>
          ...
          {isAdmin ? (
            ...
            <View
              style={[
                styles.votedBanner,
                {
                  backgroundColor: isDark ? "#1C1C2E" : "#F3F4F6",
                  borderColor: borderCol,
                },
              ]}
            >
              <Feather name="lock" size={14} color={muted} />
              <Text style={[styles.votedBannerText, { color: muted }]}>
                Poll is {st.label.toLowerCase()}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={[styles.sheet, { backgroundColor: sheetBg }]}>
          ...
          <View style={[styles.sheetHeader, { borderBottomColor: borderCol }]}>
            <View style={styles.sheetHeaderLeft}>
              <View
                style={[styles.sheetIconWrap, { backgroundColor: "#8B5CF615" }]}
              >
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {c.description ? (
                      <Text
                        style={[styles.candidateDesc, { color: muted }]}
                        numberOfLines={2}
                      >
                        {c.description}
                      </Text>
...
export default function ElectionPolls() {
  ...
  const pollsKey = queryKeys.resident.electionPolls(user?.communityId ?? "");
  ...
  const {
    ...
    queryKey: pollsKey,
    queryFn: () =>
      fetchResidentElectionPolls(token, user!.communityId as string),
  ...
  const createMutation = useMutation({
    mutationFn: (data: object) =>
      createResidentElectionPoll(token, {
        ...data,
  ...
  const voteMutation = useMutation({
    mutationFn: ({
      ...
      voteOnResidentElectionPoll(token, pollId, {
        candidateId,
        userId: user?.id,
      }),
    onSuccess: () => {
    ...
    onError: () => showError("Failed to cast vote."),
  });

  const deleteMutation = useMutation({
    mutationFn: (pollId: string) => deleteResidentElectionPoll(token, pollId),
    onSuccess: () => {
  ...
  const handleVote = (pollId, candidateId) => {
    voteMutation.mutate({ pollId, candidateId });
  };
  const confirmDelete = () => {
    setDeleting(true);
          ...
          <View style={styles.statsGrid}>
            {[
              ...
              >
                ...
                <Text style={[styles.statLabel2, { color: muted }]}>
                  {s.label}
                </Text>
              </View>
            ))}
