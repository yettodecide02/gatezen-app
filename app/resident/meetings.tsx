...
export default function CommunityMeetings() {
  ...
  const { upcoming, past } = useMemo(() => {
    ...
    u.sort((a, b) => new Date(a.scheduledAt ?? a.startTime ?? a.date).getTime() - new Date(b.scheduledAt ?? b.startTime ?? b.date).getTime());
    p.sort((a, b) => new Date(b.scheduledAt ?? b.startTime ?? b.date).getTime() - new Date(a.scheduledAt ?? a.startTime ?? a.date).getTime());
    return { upcoming: u, past: p };
  }, [meetings]);

  const displayList = tab === "upcoming" ? upcoming : past;

  const handleRsvp = async (meetingId: string, response: string) => {
    setRsvping(meetingId + response);
    try {
      const [token, communityId] = await Promise.all([getToken(), getCommunityId()]);
      await axios.post(`${config.backendUrl}/meetings/${meetingId}/rsvp`,
        { response, userId, communityId },
        { headers: token ? { Authorization: `Bearer ${token}` } : {} },
      );
      showSuccess("RSVP saved.");
      load();
    } catch { showError("Failed to update RSVP."); }
    ...
    <View style={{ flex: 1, backgroundColor: bg }}>
      ...
      <View style={{ paddingTop: Math.max(insets.top, 16), paddingBottom: 14, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: borderCol, flexDirection: "row", alignItems: "center", gap: 12 }}>
        ...
        <View>
          ...
          <Text style={{ fontSize: 12, color: muted, marginTop: 1 }}>{upcoming.length} upcoming</Text>
        </View>
      </View>
        ...
        >
          {displayList.length === 0 ? (
            <View style={{ alignItems: "center", paddingVertical: 60, gap: 10 }}>
              ...
              <Text style={{ fontSize: 15, color: muted }}>No {tab} meetings</Text>
            </View>
          ) : displayList.map((m) => {
            ...
            const totalCount = m.rsvpCounts
              ? Object.values(m.rsvpCounts as Record<string, number>).reduce((a, b) => a + b, 0)
              : goingCount;
            return (
              <View key={m.id} style={{ backgroundColor: cardBg, borderRadius: 18, borderWidth: 1, borderColor: borderCol, overflow: "hidden" }}>
                ...
                <View style={{ padding: 16, gap: 12 }}>
                  {/* Title + datetime */}
                  <View style={{ gap: 4 }}>
                    <Text style={{ fontSize: 17, fontWeight: "700", color: text }}>{m.title}</Text>
                  ...
                  {/* Description */}
                  {m.description && (
                    <Text style={{ fontSize: 13, color: muted, lineHeight: 18 }}>{m.description}</Text>
                  )}

                  {/* Agenda */}
                  {m.agenda && Array.isArray(m.agenda) && m.agenda.length > 0 && (
                    <View style={{ gap: 6 }}>
                      <Text style={{ fontSize: 11, fontWeight: "700", color: muted, textTransform: "uppercase", letterSpacing: 0.8 }}>Agenda</Text>
                      {m.agenda.map((item: string, i: number) => (
                        <View key={i} style={{ flexDirection: "row", gap: 8 }}>
                          <Text style={{ fontSize: 12, color: tint, fontWeight: "700" }}>{i + 1}.</Text>
                          <Text style={{ fontSize: 13, color: muted, flex: 1 }}>{item}</Text>
                        </View>
                      ))}
                    </View>
                  )}
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    ...
                    <Text style={{ fontSize: 12, color: muted }}>
                      {goingCount} going{totalCount > 0 ? ` of ${totalCount} responded` : ""}
                    </Text>
                  </View>
                  ...
                  {isUpcoming(dateStr) && (
                    <View>
                      <Text style={{ fontSize: 11, fontWeight: "700", color: muted, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>Your RSVP</Text>
                      <View style={{ flexDirection: "row", gap: 8 }}>
                        {RSVP_OPTIONS.map((opt) => {
                          const isSelected = myRsvp?.toUpperCase() === opt.key;
                          const isLoading = rsvping === m.id + opt.key;
                          return (
                            <Pressable key={opt.key} onPress={() => handleRsvp(m.id, opt.key)} disabled={!!rsvping}
                              style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5, paddingVertical: 9, borderRadius: 10, borderWidth: 1.5, borderColor: isSelected ? opt.color : borderCol, backgroundColor: isSelected ? opt.color + "20" : "transparent" }}
                            >
                              {isLoading ? <ActivityIndicator size="small" color={opt.color} /> : (
                                <>
                                  <Feather name={opt.icon} size={14} color={isSelected ? opt.color : muted} />
                                  <Text style={{ fontSize: 11, fontWeight: "700", color: isSelected ? opt.color : muted }}>{opt.label}</Text>
                                </>
                              )}
                            </Pressable>
                          );
                  ...
                  {!isUpcoming(dateStr) && myRsvp && (
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                      <Text style={{ fontSize: 12, color: muted }}>Your response:</Text>
                      <Text style={{ fontSize: 12, fontWeight: "700", color: RSVP_OPTIONS.find((o) => o.key === myRsvp?.toUpperCase())?.color ?? muted }}>
                        {RSVP_OPTIONS.find((o) => o.key === myRsvp?.toUpperCase())?.label ?? myRsvp}
                      </Text>
                    </View>
                  )}
                </View>
