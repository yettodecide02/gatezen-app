...
import { fetchResidentVisitors } from "@/lib/queries/resident";
      ...
      <View style={styles.cardTop}>
        ...
        <View
          style={[
            ...
            { backgroundColor: isDark ? severity.darkBg : severity.bg },
          ]}
        >
      ...
      >
        <View style={styles.durationRow}>
          <View style={styles.durationItem}>
            <Text
              style={[styles.durationLabel, { color: severity.color + "99" }]}
            >
              INSIDE FOR
            </Text>
            <Text style={[styles.durationValue, { color: severity.color }]}>
...
export default function ResidentOverstayAlert() {
  ...
  const toISO = new Date(fy, fm - 1, fd, 23, 59, 59).toISOString();
  ...
  const {
    ...
    queryKey: queryKeys.resident.visitors(
      user?.id ?? "",
      user?.communityId ?? "",
      fromISO,
      toISO,
  ...
  const visitors = Array.isArray(visitorsRaw)
    ? visitorsRaw
    : (visitorsRaw?.visitors ?? []);
  const refreshing = isFetching && !loading;

  const checkedInVisitors = useMemo(
    () => visitors.filter(isCheckedIn),
    [visitors, ticker],
  );
        ...
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            <Text style={[styles.headerSub, { color: muted }]}>
              Your visitors currently inside
            </Text>
          </View>
        </View>
        <TouchableOpacity
        ...
        >
          ...
          <View style={{ flexDirection: "row", gap: 12 }}>
            ...
            >
              ...
              <Text
                style={[
                  styles.summaryValue,
                  { color: overstays.length > 0 ? "#EF4444" : textColor },
                ]}
              >
                {overstays.length}
              </Text>
          ...
          {overstays.length > 0 && (
            ...
            >
              ...
              >
                {overstays.length} of your visitor
                {overstays.length > 1 ? "s have" : " has"} exceeded the allowed
                time. Please ask them to check out.
              </Text>
            </View>
          )}
          ...
          {overstays.length === 0 ? (
            ...
            >
              ...
              <Text style={[styles.allClearTitle, { color: textColor }]}>
                All Clear!
              </Text>
            ...
            {Object.entries(OVERSTAY_LIMITS).map(([type, mins], i, arr) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={[styles.limitValue, { color: cfg.color }]}>
                    {formatDuration(mins)}
                  </Text>
                </View>
              );
