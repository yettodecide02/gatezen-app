...
import {
  fetchResidentParkingAll,
  bookResidentParkingSpot,
  cancelResidentParkingBooking,
...
function spotTypeCfg(t = "4W") {
  return SPOT_TYPE_CFG[t] ?? SPOT_TYPE_CFG["4W"];
}
...
export default function RentParking() {
  ...
  const parkingKey = queryKeys.resident.parkingAll(
    user?.communityId ?? "",
    user?.id ?? "",
  );
  ...
  const {
    ...
    queryKey: parkingKey,
    queryFn: () =>
      fetchResidentParkingAll(token, user!.communityId as string, user!.id),
    enabled: !!user?.communityId && !!user?.id,
    staleTime: 5 * 60 * 1000,
  });
  const { spots = [], bookings = [] } = parkingData ?? {};
  const refreshing = isFetching && !loading;
  const availableSpots = useMemo(
    () =>
      spots.filter((s) => s.isAvailable !== false && s.status !== "OCCUPIED"),
    [spots],
  );

  const bookMutation = useMutation({
    mutationFn: (payload: object) => bookResidentParkingSpot(token, payload),
    onSuccess: () => {
      showSuccess("Parking spot booked!");
      setSelectedSpot(null);
      setTab("mybookings");
      queryClient.invalidateQueries({ queryKey: parkingKey });
    },
    onError: (e: any) =>
  ...
  const cancelMutation = useMutation({
    mutationFn: (id: string) => cancelResidentParkingBooking(token, id),
    onSuccess: () => {
                  ...
                  >
                    ...
                    <View
                      style={{
                        ...
                        paddingVertical: 3,
                        borderRadius: 8,
                        backgroundColor: isActive ? "#10B98120" : "#94A3B820",
                      }}
                    >
                      <Text
                        style={{
          ...
          <View style={{ flex: 1, backgroundColor: bg }}>
            ...
            >
              <Text style={{ fontSize: 18, fontWeight: "700", color: text }}>
                Book Spot {selectedSpot.spotNumber ?? selectedSpot.slotNumber}
              </Text>
            ...
            <View style={{ padding: 20, gap: 16 }}>
                      ...
                      DateTimePickerAndroid.open({
                        ...
                        onChange: (e, d) => {
                          if (e.type === "set" && d) {
                            setFromDate(d);
                            if (d > toDate)
                              setToDate(new Date(d.getTime() + 86400000));
                          }
                        },
                      });
                    } else {
              ...
              {selectedSpot.pricePerDay != null && (
                ...
                >
                  ...
                  <Text
                    style={{ fontSize: 16, fontWeight: "800", color: tint }}
                  >
                    ₹
