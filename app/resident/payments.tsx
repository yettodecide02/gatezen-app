...
import { fetchResidentPayments } from "@/lib/queries/resident";
...
export default function Payments() {
  ...
  const { data: rawItems = [], isLoading: loading } = useQuery({
    queryKey: queryKeys.resident.payments(
      user?.id ?? "",
      user?.communityId ?? "",
    ),
    queryFn: () =>
      fetchResidentPayments(token, user!.id, user!.communityId as string),
  ...
  const fmtAmt = (amt, cur) =>
    `${cur === "INR" ? "₹" : "$"}${Number(amt).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`;
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 12, color: muted, marginTop: 2 }}>
              {fmt(item.createdAt)}
              {item.paidAt ? ` · Paid ${fmt(item.paidAt)}` : ""}
            </Text>
          </View>
          <Text style={{ fontSize: 16, fontWeight: "700", color: sc.color }}>
        ...
        {item.status === "due" && (
          <Pressable
            onPress={() =>
              showInfo(
                "Online payment will be available soon. Please contact admin for manual payment.",
              )
            }
            style={({ pressed }) => ({
              alignSelf: "flex-end",
              ...
              paddingVertical: 8,
              borderRadius: 10,
              backgroundColor: pressed ? tint + "CC" : tint,
            })}
          >
        ...
        {item.status === "paid" && (
          <Pressable
            onPress={() => showInfo("Receipt download coming soon.")}
            style={{
              alignSelf: "flex-end",
              flexDirection: "row",
              alignItems: "center",
        ...
        >
          ...
          {due.length > 0 && (
            ...
            >
              ...
              >
                You have {due.length} pending payment
                {due.length !== 1 ? "s" : ""}. Please clear dues soon.
              </Text>
            </View>
          )}
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: 12, color: muted, marginTop: 1 }}>
                Auto-process monthly dues
              </Text>
            </View>
            <Switch
              value={autopay}
              onValueChange={setAutopay}
              trackColor={{ false: borderCol, true: tint + "60" }}
              thumbColor={autopay ? tint : muted}
            />
          </View>
