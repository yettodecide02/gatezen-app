...
const STATUS_CONF = {
  ...
  APPROVED: { color: "#10B981", label: "APPR" },
  REJECTED: { color: "#EF4444", label: "RJCT" },
  CHECKED_IN: { color: "#3B82F6", label: "IN" },
...
export default function KidPasses() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${config.backendUrl}/resident/kid-passes?userId=${user.id}&communityId=${user.communityId}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      setKidPasses(Array.isArray(res.data) ? res.data : []);
    } catch {
      showError("Failed to load kid passes");
    } finally {
      setLoading(false);
    }
  }, [user, token]);
  ...
  const handleCreate = async () => {
    if (!form.childName.trim()) {
      showError("Child name is required");
      return;
    }
    ...
    const digitsOnly = form.contact.replace(/[\s\-\+\(\)]/g, "");
    ...
    try {
      ...
      await axios.post(
        `${config.backendUrl}/resident/kid-passes`,
        {
      ...
      showError(e.response?.data?.error || "Failed to create kid pass");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        {loading ? (
          ...
          <View style={{ alignItems: "center", paddingVertical: 40, gap: 8 }}>
            ...
            <Text style={{ fontSize: 13, color: muted, textAlign: "center" }}>
              Create a pass to allow supervised child exit.
            </Text>
            <Pressable
              onPress={() => setShowCreateModal(true)}
              style={{
          ...
          kidPasses.map((kp) => {
            ...
            return (
              ...
              >
                ...
                <Text style={{ fontSize: 12, color: muted }}>
                  {kp.childAge ? `Age ${kp.childAge} · ` : ""}Parent:{" "}
                  {kp.parentName} · {kp.contact}
                </Text>
                <Text style={{ fontSize: 11, color: muted }}>
                  Valid: {fmt(kp.validFrom)} — {fmt(kp.validTo)}
                </Text>
                {kp.permissions && (
                  <Text style={{ fontSize: 11, color: tint }}>
                    Permissions: {kp.permissions}
                  </Text>
                )}
              </View>
            );
