...
export default function Passes() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      showError("Failed to load visitor passes");
    } finally {
      setLoading(false);
    }
  }, [user, token, from, to]);
      ...
      >
        ...
        >
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
              ...
              onPress={() => {
                if (Platform.OS === "android") {
                  DateTimePickerAndroid.open({
                    ...
                    mode: "date",
                    maximumDate: new Date(to + "T00:00:00"),
                    onChange: (e, d) => {
                      if (e.type === "set" && d) setFrom(isoDate(d));
                    },
                  });
                } else {
            ...
            <Text style={{ alignSelf: "center", color: muted, fontSize: 12 }}>
              —
            </Text>
            <Pressable
              onPress={() => {
                if (Platform.OS === "android") {
                  DateTimePickerAndroid.open({
                    ...
                    mode: "date",
                    minimumDate: new Date(from + "T00:00:00"),
                    maximumDate: new Date(),
                    onChange: (e, d) => {
                      if (e.type === "set" && d) setTo(isoDate(d));
                    },
                  });
                } else {
        ...
        {loading ? (
          ...
          <View style={{ alignItems: "center", paddingVertical: 40, gap: 8 }}>
            ...
            <Text style={{ fontSize: 13, color: muted }}>
              No visitor passes in this date range.
            </Text>
          </View>
        ) : (
          visitors.map((v) => {
